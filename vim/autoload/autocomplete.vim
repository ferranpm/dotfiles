...
function! autocomplete#completions(regexp)
  ...
  for l:mode in split(&complete, ',')
    ...
    elseif l:mode == 'w'
      call autocomplete#add_from_buffers(filter(l:buffers, '!empty(v:val["windows"]) && v:val["bufnr"] != bufnr()'), a:regexp, 'w')
    elseif l:mode == 'b'
      call autocomplete#add_from_buffers(filter(l:buffers, 'empty(v:val["windows"])'), a:regexp, 'b')
...
function! autocomplete#add_from_buffers(buffers, regexp, mode)
  for l:buffer in a:buffers
    ...
    if get(l:buffer, "linecount") > 5000
      continue
    endif
...
function! autocomplete#add_from_lines(lines, regexp, mode)
